...
func (c *TestClient) GetJSON(path string) *http.Response {
	req, err := http.NewRequest("GET", c.BaseURL+path, nil)
	if err != nil {
		c.t.Fatalf("failed to create GET request: %v", err)
	}
...
// PostJSON performs a POST request with JSON data and Accept: application/json header
func (c *TestClient) PostJSON(path string, data url.Values) *http.Response {
	req, err := http.NewRequest("POST", c.BaseURL+path, strings.NewReader(data.Encode()))
	if err != nil {
		c.t.Fatalf("failed to create POST request: %v", err)
	}
